...
var DEFAULT_PATH_PREFIX = '/databus/consumer-service/v1'
...
function Channel (base, consumerGroup, options) {
  if (!base) {
  ...
  this._offset = options.offset || 'latest'
  var offsetValues = ['latest', 'earliest', 'none']
  if (!this._offset || offsetValues.indexOf(this._offset) < 0) {
    throw new PermanentError('Value for \'offset\' must be one of ' +
      offsetValues.join(', '))
  }

  this._pathPrefix = options.pathPrefix || DEFAULT_PATH_PREFIX

  this._timeout = options.timeout || DEFAULT_CHANNEL_TIMEOUT
