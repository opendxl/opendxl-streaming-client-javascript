  ...
  this._run = function (processCallback, doneCallback, waitBetweenQueries) {
    ...
    this.consume(function (consumeError, payloads) {
      if (consumeError) {
        ...
        try {
          processCallback(payloads, function (processError) {
            if (processError) {
              doneCallback(processError)
            } else {
              channel.commit(function (commitError) {
                if (commitError) {
                  doneCallback(commitError)
                } else {
                  setTimeout(doneCallback, waitBetweenQueries * 1000)
                }
              })
            }
          })
