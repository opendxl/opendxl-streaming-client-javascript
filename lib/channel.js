  ...
  var doRun = function () {
    channel._run(processCallback,
      function (error, continueRunning) {
        if (!error && continueRunning) {
          doRun()
        } else {
          channel._running = false
          channel._stopCallbacks.forEach(function (stopCallback) {
            try {
              stopCallback()
            } catch (stopError) {
          ...
          channel._stopCallbacks = []
          channel._stopRequested = false
...
Channel.prototype.destroy = function (callback) {
  if (this._active) {
    ...
    this.stop(function (stopCallback) {
      channel.delete(function (deleteCallback) {
